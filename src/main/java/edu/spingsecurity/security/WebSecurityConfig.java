...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.csrf().disable()
            ...
            .and()
            .formLogin()
            .loginPage("/login.html").loginProcessingUrl("/login").failureUrl("/deny.html")
            .and().anonymous().authorities("ROLE_ANON").principal(UserDetailsAdapter.anonymous());
            //.and().rememberMe().rememberMeServices(new TokenBasedRememberMeServices("hello-key", userDetailsService));
